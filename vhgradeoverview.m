...
if ~isempty(after) && isfield(after,'Autograded_only') && after.Autograded_only
    fprintf(['[vhgradeoverview] auto-saved (no UI needed) — %s / %s : %g of %g\n' ...
             '   %s\n'], ...
             s.students{row}, item.Item_name, ...
             after.Points_earned, after.Points_possible, ...
             strrep(commentSummary(after), char(10), ' | '));
end
end
